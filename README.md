...
本番モードでは `/auth` の「新規登録」から、ユーザー本人が名前・メールアドレス・パスワードを登録できます。
登録時に Firebase Authentication のアカウントを作成し、サーバー側で `users/{uid}` を `role: "member"`、`isActive: false` として作成します。
管理者が `/admin/users` で対象ユーザーを「有効」にするまで、イベント情報にはアクセスできません。
自己登録APIは管理者ロールを受け付けず、Firestore Rulesでもクライアントから `users` を直接書き換えることはできません。
